...
CSideHair::CSideHair() : CShadowVolume() {}
...
void CSideHair::RenderLine(vec3_t v1, vec3_t v2, vec3_t c1, vec3_t c2) {
	...
	VectorScale(vOrtho, 10.0f, vOrtho);
	glBegin(GL_QUADS);
	glTexCoord2f(0.0f, 0.0f + fTextureMove + fTextureV);
	glVertex3f(p1[0] - vOrtho[0], p1[1] - vOrtho[1], p1[2] - vOrtho[2]);
	glTexCoord2f(0.0f, 1.0f - fTextureMove + fTextureV);
	glVertex3f(p2[0] - vOrtho[0], p2[1] - vOrtho[1], p2[2] - vOrtho[2]);
	glTexCoord2f(1.0f, 1.0f - fTextureMove + fTextureV);
	glVertex3f(p2[0] + vOrtho[0], p2[1] + vOrtho[1], p2[2] + vOrtho[2]);
	glTexCoord2f(1.0f, 0.0f + fTextureMove + fTextureV);
	glVertex3f(p1[0] + vOrtho[0], p1[1] + vOrtho[1], p1[2] + vOrtho[2]);
	glEnd();
}
