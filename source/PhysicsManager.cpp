// PhysicsManager.cpp [2.0] : implementation of the CPhysicsManager class.
//////////////////////////////////////////////////////////////////////
...
void CPhysicsVertex::UpdateForce(unsigned int iKey, DWORD dwType, float fWind)
{
	...
	float fRand = static_cast<float>(iTemp == 0 ? 0 : iTemp + 2);
	...
	case PCT_ELASTIC_HALLOWEEN:
		...
		m_vForce[2] += fRand * (fWind + 0.1f) * 0.5f * static_cast<float>(sinf(WorldTime * 0.003f)) * 5.0f;
...
void CPhysicsVertex::DoOneTimeMove(void)
{
	...
	if (m_iCountOneTimeMove > 0)
	{
		for (int i = 0; i < 3; ++i)
		{
			m_vOneTimeMove[i] /= static_cast<float>(m_iCountOneTimeMove);
		}
...
BOOL CPhysicsCloth::Create(OBJECT* o, int iBone, float fxPos, float fyPos, float fzPos, int iNumHor, int iNumVer, float fWidth, float fHeight, int iTexFront, int iTexBack, DWORD dwType)
{
	...
	m_fUnitWidth = m_fWidth / static_cast<float>(m_iNumHor - 1);
	m_fUnitHeight = m_fHeight / static_cast<float>(m_iNumVer - 1);
	int iLink = 0;
	float Matrix[3][4];
	...
	for (int j = 0; j < m_iNumVer; ++j)
	{
		...
		case PCT_SHORT_SHOULDER:
			fWidth *= RATE_SHORT_SHOULDER + (1.0f - RATE_SHORT_SHOULDER) * static_cast<float>(j) / static_cast<float>(m_iNumVer - 1);
			fUnitWidth = fWidth / static_cast<float>(m_iNumHor - 1);
			break;
		case PCT_CYLINDER:
			bCylinder = true;
			break;
		...
		for (int i = 0; i < m_iNumHor; ++i)
		{
			...
			if (bCylinder)
			{
				float fPosY = sinf(static_cast<float>(i) / static_cast<float>(m_iNumHor - 1) * 3.14f) * 100.0f;

				vPos[0] = (fUnitWidth * static_cast<float>(i));
				vPos[1] = fPosY - 30.0f;
				vPos[2] = (fUnitHeight * static_cast<float>(j));
			}
			else
			{
				vPos[0] = (fUnitWidth * static_cast<float>(i)) - 0.5f * fWidth;
				vPos[1] = 20.0f;
				vPos[2] = -fUnitHeight * static_cast<float>(j);
			}
			...
			case PCT_CURVED:
			{
				float fMove = 2.0f * fabs(static_cast<float>(i) / static_cast<float>(m_iNumHor - 1) - 0.5f);
...
void CPhysicsCloth::SetFixedVertices(float Matrix[3][4])
{
	...
	for (int iVertex = 0; iVertex < m_iNumHor; ++iVertex)
	{
		...
		{
		case PCT_CURVED:
			float fMove = 2.0f * fabs((float)iVertex / (float)(m_iNumHor - 1) - 0.5f);
			vPos[1] -= 10.0f * fMove * fMove;
			break;
		}
...
{
	St_PhysicsLink& link = m_pLink[iLink];
...
BOOL CPhysicsCloth::Move(float fTime)
{
	...
	{
	case PCT_RUBBER2:
		m_fWind = static_cast<float>((rand() % m_byWindMax + m_byWindMin)) / 100.0f;
		break;
	default:
		if (gMapManager.WorldActive == 55)
			m_fWind = static_cast<float>(rand() % 40 + 10) / 50.0f;
		else
	...
	{
	case PCT_CYLINDER:
		m_fWind = static_cast<float>(rand() % 10 + 25) / 50.0f;
		break;
	}
...
BOOL CPhysicsCloth::PreventFromStretching()
{
#ifdef ADD_COLLISION
	CNode<CPhysicsCollision*>* pHead = m_lstCollision.FindHead();
	for (; pHead; pHead = m_lstCollision.GetNext(pHead))
...
void CPhysicsCloth::RenderVertex(vec3_t* pvRenderPos, int xVertex, int yVertex)
{
	...
	vec3_t* pvPos = &pvRenderPos[iVertex];
	glTexCoord2f((float)xVertex / (float)(m_iNumHor - 1), (float)yVertex / (float)(m_iNumVer - 1));
	glVertex3fv(*pvPos);
}

void CPhysicsCloth::RenderCollisions()
{
	...
	for (; pHead; pHead = m_lstCollision.GetNext(pHead))
	{
		...
		{
			CPhysicsColSphere* pColSph = static_cast<CPhysicsColSphere*>(pCol);

			static GLUquadricObj* pQuad = nullptr;
			if (nullptr == pQuad)
			{
				pQuad = gluNewQuadric();
			}
...
void CPhysicsManager::RemoveAll()
{
	CNode<CPhysicsCloth*>* pNode = m_lstCloth.FindHead();
	while (pNode)
	{
		CPhysicsCloth* pCloth = pNode->GetData();
		pNode = m_lstCloth.GetNext(pNode);
		pCloth->Destroy();
		delete pCloth;
	}
