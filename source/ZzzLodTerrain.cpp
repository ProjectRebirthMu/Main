...
float RequestTerrainHeight(float xf, float yf) {
	...
	float left = BackTerrainHeight[Index1] +
		(BackTerrainHeight[Index2] - BackTerrainHeight[Index1]) * yd;
	float right = BackTerrainHeight[Index3] +
		(BackTerrainHeight[Index4] - BackTerrainHeight[Index3]) * yd;
...
void CreateLodBuffer() {
	for (int y = 0; y < TERRAIN_SIZE; y += 4) {
		for (int x = 0; x < TERRAIN_SIZE; x += 4) {
			vec3_t NormalMin = { 1.f, 1.f, 1.f };
			vec3_t NormalMax = { -1.f, -1.f, -1.f };
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 4; j++) {
					vec3_t v1, v2, v3, v4;
			...
			float Delta = max(max(abs(NormalMax[0] - NormalMin[0]), abs(NormalMax[1] - NormalMin[1])), abs(NormalMax[2] - NormalMin[2]));
...
void InterpolationHeight(int lod, int x, int y, int xd, float* TerrainHeight) {
	if (lod >= 4) {
		TerrainHeight[TERRAIN_INDEX_REPEAT(x + xd, y + 2)] = (TerrainHeight[TERRAIN_INDEX_REPEAT(x + xd, y)] + TerrainHeight[TERRAIN_INDEX_REPEAT(x + xd, y + 4)]) * 0.5f;
	}
	if (lod >= 2) {
		TerrainHeight[TERRAIN_INDEX_REPEAT(x + xd, y + 1)] = (TerrainHeight[TERRAIN_INDEX_REPEAT(x + xd, y)] + TerrainHeight[TERRAIN_INDEX_REPEAT(x + xd, y + 2)]) * 0.5f;
		TerrainHeight[TERRAIN_INDEX_REPEAT(x + xd, y + 3)] = (TerrainHeight[TERRAIN_INDEX_REPEAT(x + xd, y + 2)] + TerrainHeight[TERRAIN_INDEX_REPEAT(x + xd, y + 4)]) * 0.5f;
	}
}

void InterpolationWidth(int lod, int x, int y, int yd, float* TerrainHeight) {
	if (lod >= 4) {
		TerrainHeight[TERRAIN_INDEX_REPEAT(x + 2, y + yd)] = (TerrainHeight[TERRAIN_INDEX_REPEAT(x, y + yd)] + TerrainHeight[TERRAIN_INDEX_REPEAT(x + 4, y + yd)]) * 0.5f;
	}
	if (lod >= 2) {
		TerrainHeight[TERRAIN_INDEX_REPEAT(x + 1, y + yd)] = (TerrainHeight[TERRAIN_INDEX_REPEAT(x, y + yd)] + TerrainHeight[TERRAIN_INDEX_REPEAT(x + 2, y + yd)]) * 0.5f;
		TerrainHeight[TERRAIN_INDEX_REPEAT(x + 3, y + yd)] = (TerrainHeight[TERRAIN_INDEX_REPEAT(x + 2, y + yd)] + TerrainHeight[TERRAIN_INDEX_REPEAT(x + 4, y + yd)]) * 0.5f;
	}
}
...
void PrefixTerrainHeightEdge(int x, int y, int lod, float* TerrainHeight) {
	...
	if (lod >= LodBuffer[((y - 1) & 63) * 64 + ((x & 63) * 64)]) {
...
void PrefixTerrainHeight() {
	for (int y = 0; y < 64; y++) {
		for (int x = 0; x < 64; x++) {
			int lod = LodBuffer[((y & 63) * 64) + ((x & 63) * 64)];
...
inline void Interpolation(int mx, int my) {
	int Index01 = (my * 2) * 512 + (mx * 2 + 1);
	int Index12 = (my * 2 + 1) * 512 + (mx * 2 + 2);
	int Index23 = (my * 2 + 2) * 512 + (mx * 2 + 1);
	int Index30 = (my * 2 + 1) * 512 + (mx * 2);
	int Index02 = (my * 2 + 1) * 512 + (mx * 2 + 1);
...
void RenderTerrain(bool EditFlag) {
	...
	if (!EditFlag) {
		// Set the water movement speed based on the world.
		if (gMapManager.WorldActive == WD_8TARKAN) {
	...
	// Disable alpha blending if not in edit mode.
	if (!EditFlag) {
		DisableAlphaBlend();
	}
	else {
		// Reset the selection flag and initialize the collision detection line to face.
		SelectFlag = false;
		InitCollisionDetectLineToFace();
	}

	// Set the terrain flag to normal and render the terrain frustum.
	TerrainFlag = TERRAIN_MAP_NORMAL;
	RenderTerrainFrustrum(EditFlag);

	// If in edit mode and the select flag is set, render the selected tile.
	if (EditFlag && SelectFlag) {
		RenderTerrainTile(SelectXF, SelectYF, (int)SelectXF, (int)SelectYF, 1.f, 1, EditFlag);
	}

	// If not in edit mode, enable alpha testing and render the grass terrain if enabled.
	if (!EditFlag) {
		EnableAlphaTest();
		...
		// Disable depth testing, enable culling, render the pointers, and then re-enable depth testing.
		DisableDepthTest();
		EnableCullFace();
		RenderPointers();
		EnableDepthTest();
