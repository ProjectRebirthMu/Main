///////////////////////////////////////////////////////////////////////////////
// Opções de IA Aprimoradas
// Melhorias foram feitas para otimizar o processamento.
///////////////////////////////////////////////////////////////////////////////
...
float CreateAngle(float x1, float y1, float x2, float y2)
{
	...
	if (fabsf(nx2) < 0.0001f)
	{
		if (ny2 < 0.f) return 0.f;   // s
		else return 180.f; // n
	}
	else if (fabsf(ny2) < 0.0001f)
	{
	...
	{
		angle = atanf(ny2 / nx2) / 3.1415926536f * 180.f + 90.f;
...
{
	float Grad;
	if (fabs(TargetX - PositionX) < 0.0001f)
		Grad = 0.f;
	else
		Grad = (PositionY - TargetY) / (TargetX - PositionX);

	int TargetTheta = static_cast<int>(atanf(Grad) * (180.f / Q_PI));
...
{
	BYTE position;
	int PositionX = static_cast<int>(x / TERRAIN_SCALE);
	int PositionY = static_cast<int>(y / TERRAIN_SCALE);
	int TargetX = static_cast<int>(Tx / TERRAIN_SCALE);
	int TargetY = static_cast<int>(Ty / TERRAIN_SCALE);

	BYTE dx = static_cast<BYTE>(8 + TargetX - PositionX);
	BYTE dy = static_cast<BYTE>(8 + TargetY - PositionY);

	position = static_cast<BYTE>((dx | (dy << 4)));
...
void Alpha(OBJECT* o)
{
	...
	o->BlendMeshLight = min(o->Alpha, o->BlendMeshLight);
}
...
void LookAtTarget(OBJECT* o, CHARACTER* tc)
{
	...
	float deltaAngle = FarAngle(o->Angle[2], Angle);
...
void MoveHead(CHARACTER* c)
{
	...
	if (o->CurrentAction != MONSTER01_DIE)
	{
		if (o->CurrentAction == MONSTER01_STOP1)
		{
			if (rand() % 32 == 0)
			{
				o->HeadTargetAngle[0] = static_cast<float>(rand() % 128 - 64);
				o->HeadTargetAngle[1] = static_cast<float>(rand() % 48 - 16);
...
void CalcFPS()
{
	...
	static int start = 0, start2 = 0, current = 0, last = 0;
	...
	double dif = static_cast<double>(current - start) / CLOCKS_PER_SEC;
	double rv = (dif) ? static_cast<double>(frame) / dif : -1.0;
	...
	DeltaT = static_cast<float>(current - last) / CLOCKS_PER_SEC;
	if (current == last)
	{
		DeltaT = 0.1f / CLOCKS_PER_SEC;
	}

	FPS = static_cast<float>(rv);
	last = current;
